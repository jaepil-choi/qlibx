...
from qlibx.optimization import (
...
class EnhancedIndexTargetPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        self.optimizer = CvxpyIntentTrackingOptimizer(OptimizerConfig(solver=config.solver))
    ...
    def __call__(
        ...
        physical_target, cash_target, normalization_delta = _normalize_numerical_solution(
            ...
            self.constituent_rows.append(
                {
                    ...
                    "realized_lookthrough_exposure": float(lookthrough_exposure[constituent]),
                }
            )
            ...
            self.physical_rows.append(
                {
                    ...
                    "current_physical_weight": float(problem.current_physical_weight[instrument]),
                    "raw_target_physical_weight": float(raw_physical_target[instrument]),
...
def _lookthrough_matrix(
    ...
    *,
    ...
    if set(matrix.index) != set(constituents.astype(str)) or set(matrix.columns) != set(
        physical.astype(str)
    ):
        raise ValueError(
