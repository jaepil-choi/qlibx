...
def build_enhanced_index_attribution(
    ...
    *,
    ...
    if backtest.run_kind != "backtest":
        raise ValueError(f"enhanced attribution input is not a backtest: {backtest_run_id}")
    config = backtest.metadata.get("backtest_config", {})
    if config.get("target_semantics") != "enhanced_index":
        raise ValueError(f"attribution input is not an enhanced-index backtest: {backtest_run_id}")
    alpha_links = [
        link for link in store.get_parent_links(backtest_run_id) if link.role == "alpha_input"
    ]
    if len(alpha_links) != 1:
        raise ValueError("enhanced-index backtest must have exactly one alpha_input parent")
    ...
    constituent = store.load_table(backtest_run_id, "optimizer_constituent_daily").copy()
    physical = store.load_table(backtest_run_id, "optimizer_physical_daily").copy()
    ...
    if missing := required_constituent.difference(constituent.columns):
        raise ValueError(f"optimizer constituent artifact is missing: {sorted(missing)}")
    if missing := required_physical.difference(physical.columns):
        raise ValueError(f"optimizer physical artifact is missing: {sorted(missing)}")
    ...
    _validate_primary_key(constituent, ["trade_date", "constituent_id"], "optimizer constituent")
    _validate_primary_key(physical, ["trade_date", "instrument_id"], "optimizer physical")
    ...
    if not np.allclose(
        ...
        raise ValueError("stored optimizer desired active exposure differs from alpha artifact")
    ...
    target_long = (
        target.rename_axis(index="trade_date", columns="instrument_id")
        .stack(future_stack=True)
        .rename("stored_target_weight")
        .reset_index()
    )
    held_long = (
        held.rename_axis(index="trade_date", columns="instrument_id")
        .stack(future_stack=True)
        .rename("held_quantity")
        .reset_index()
    )
    ...
    if not np.allclose(
        ...
        raise ValueError("stored optimizer physical target differs from execution target artifact")
    return EnhancedIndexAttributionResult(
        backtest_run_id=backtest_run_id,
        alpha_run_id=alpha_run_id,
        member_intent_daily=member_intent,
        constituent_daily=constituent.sort_values(["trade_date", "constituent_id"]).reset_index(
            drop=True
        ),
        physical_daily=physical.sort_values(["trade_date", "instrument_id"]).reset_index(drop=True),
    )
...
def _member_intent(
    ...
    *,
    ...
    links = [link for link in store.get_parent_links(alpha_run_id) if link.role == "member"]
    if links:
    ...
    for member_run_id in sorted(members):
        ...
        if not alpha.index.equals(desired.index) or not alpha.columns.equals(desired.columns):
            raise ValueError(f"enhanced-index member alpha axes differ: {member_run_id}")
        weighted = alpha.shift(signal_lag, fill_value=0.0) * members[member_run_id]
        combined = combined + weighted
        long = (
            weighted.rename_axis(index="trade_date", columns="constituent_id")
            .stack(future_stack=True)
            .rename("weighted_active_exposure")
            .reset_index()
        )
    ...
    if not np.allclose(
        ...
        raise ValueError("stored member alpha lineage does not sum to desired active exposure")
...
def _held_quantity(
    ...
    *,
    ...
    return (
        ...
        .reindex(index=index, columns=columns, fill_value=0.0)
        .fillna(0.0)
    )
