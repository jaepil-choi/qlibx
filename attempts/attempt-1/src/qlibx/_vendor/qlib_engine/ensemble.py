...
def build_ensemble(
    ...
    *,
    ...
    config_fingerprint = stable_hash({"strategy_id": strategy_id, "members": member_definition})
    dataset_fingerprint = stable_hash(
        {run_id: store.get_run(run_id).dataset_fingerprint for run_id in sorted(normalized_members)}
    )
    ...
    if not store.has_complete(alpha_run_id):
        store.publish(
            ...
            parents=tuple(
                ParentLink(run_id, "member", weight) for run_id, weight in member_definition
            ),
        )
...
def _load_execution_context(
    ...
    return ExecutionContext(
        ...
        observed=(None if matched is None else store.load_table(backtest_run_id, "observed")),
        tradable=(None if matched is None else store.load_table(backtest_run_id, "tradable")),
        shortable=(None if matched is None else store.load_table(backtest_run_id, "shortable")),
        volume=(
            None if config.execution.volume is None else store.load_table(backtest_run_id, "volume")
        ),
    )
