...
def run_qlib_backtest(
    ...
    volume = (
        ...
        else _align_matrix(context.volume, context.execution_price, "volume").astype("float64")
    )
    tradable = (
        None
        if context.tradable is None
        else _align_matrix(context.tradable, context.execution_price, "tradable").astype(bool)
    )
    ...
    if config.target_semantics == "signed_weight":
        from qlibx._vendor.qlib_backend.backend import MatchedCapitalizationInputs
        ...
        matched_inputs = MatchedCapitalizationInputs(
            ...
            observed=_align_matrix(context.observed, context.execution_price, "observed").astype(
                bool
            ),
            shortable=_align_matrix(context.shortable, context.execution_price, "shortable").astype(
                bool
            ),
    ...
    artifacts = {
        ...
        "valuation_price": (
            context.execution_price if context.valuation_price is None else scenario.valuation_price
        ),
        "position_unit_factor": factor,
        "universe": universe,
        "benchmark_weight": benchmark,
        ...
        artifacts.update(
            {
                ...
                "optimizer_constituent_daily": pd.DataFrame(enhanced_policy.constituent_rows),
                "optimizer_physical_daily": pd.DataFrame(enhanced_policy.physical_rows),
            }
        )
