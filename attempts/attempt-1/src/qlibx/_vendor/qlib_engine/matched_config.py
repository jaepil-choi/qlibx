...
def parse_matched_capitalization_config(
    ...
    *,
    ...
    if unknown:
        raise error_type(f"unknown backtest.matched_capitalization keys: {unknown}")
    return MatchedCapitalizationConfig(
        ...
        per_name_short_cap=float(_required(raw, "per_name_short_cap", error_type=error_type)),
        ...
        active_booksize=(
            None if raw.get("active_booksize") is None else float(raw["active_booksize"])
        ),
    )
...
def validate_matched_capitalization_config(
    ...
    *,
    ...
    if target_semantics == "signed_weight" and config is None:
        raise error_type("signed_weight target_semantics requires matched_capitalization")
    if target_semantics != "signed_weight" and config is not None:
        raise error_type("matched_capitalization requires signed_weight target_semantics")
    if config is None:
        return
    if not 0 < config.per_name_short_cap <= 1:
        raise error_type("matched_capitalization per_name_short_cap must be in (0, 1]")
    if config.safety_multiplier < 1:
        raise error_type("matched_capitalization safety_multiplier must be at least 1")
    if config.inventory_readiness != "same_bar":
        raise error_type("matched_capitalization inventory_readiness currently supports same_bar")
    if config.inventory_retention not in {"retained", "active_short_only"}:
        raise error_type(
            "matched_capitalization inventory_retention must be retained or active_short_only"
        )
    if config.active_booksize is not None and not (0 < config.active_booksize <= initial_cash):
        raise error_type(
