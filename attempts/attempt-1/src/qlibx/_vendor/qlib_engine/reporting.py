        ...
        rows.append(
            {
                ...
                "total_return": float(
                    ...
                    if is_signed
                    else (1.0 + account["portfolio_return"].astype(float)).prod() - 1.0
                ),
...
def _render_html(metrics: pd.DataFrame) -> str:
    run_ids = "\n".join(
        f"<li><code>{html.escape(str(run_id))}</code></li>" for run_id in metrics["run_id"]
    )
...
def _write_cumulative_return_figure(accounts: dict[str, pd.DataFrame], path: Path) -> None:
    import matplotlib
    ...
    for run_id, account in accounts.items():
        ...
        cumulative = (
            returns.cumsum() if "return_denominator" in account else (1.0 + returns).cumprod() - 1.0
        )
