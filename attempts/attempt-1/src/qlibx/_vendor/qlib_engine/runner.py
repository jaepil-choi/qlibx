...
def _context_from_catalog(project: ProjectConfig, data: MatrixCatalog) -> ExecutionContext:
    ...
    return ExecutionContext(
        ...
        valuation_price=(
            None if config.valuation_price is None else data.load(config.valuation_price)
        ),
        position_unit_factor=(
            None if config.position_unit_factor is None else data.load(config.position_unit_factor)
        ),
        ...
        volume=(None if config.execution.volume is None else data.load(config.execution.volume)),
    )
