...
**Status:** **CLOSED 2026-09-04 -- the design half by owner ruling (one-shape campaign, decision A1).** A strategy is one file. A settings family is a file family, even when the members differ by a constant: the record reads a re-registered file as a *tuning* of the same strategy (same name, new fingerprint), and an author who does not mean that -- a Fama-French arm is a different strategy, not a tuned one -- writes a second file. Which of the two is meant is the authoring agent's call, not a framework channel; `ComponentDocument.config` stays unwired and no `--config` is added. The skill states the rule beside the one-subclass rule. **Docs half, earlier the same day:** DOCS HALF CLOSED on `fix/0.4.0-open-issues`, record `docs/implementations/149-the-open-issues-at-0.4.0.md`: docs half: `Model.inputs`'s docstring and the skill say `inputs()` is evaluated at registration and preflight before any memory is applied, so the reads cannot depend on memory or per-model settings. The design half (one class, several ids with a config) stays OPEN for an owner decision.
