...
| `036` | 둘은 같은 방식으로 저작되지 않는다 | **owner-decided CONVERGE. 세 역할 모두 class 하나 — Constraint(`130`)·DataModel(`131`)·StrategyModel(`132`).** 표 10행 전부 닫혔다(5–9행 `128`, lookback `126`, 1–4·10행 `131`·`132`). `_internal/strategy_bridge.py`와 `_internal/models/`는 삭제됐다. **남은 것은 M1.4** — `SKILL.md`의 *"Both are authored the same way"*를 저자의 눈으로 다시 읽고 이 이슈를 닫는 것 | 명사 2 (Surface) — **소유자가 착수 1순위로 정했다** |
