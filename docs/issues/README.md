...
**작성 2026-09-02 · 갱신 2026-09-05.** 이 디렉터리에 86개 파일이 있고 **75개가 닫혔다.** 닫힌 파일을 옮기지 않는
...
| # | 판정 | 한 줄 |
|---|---|---|
| `078` | 두 번째 절반: **거부하지 않는다** | 지불 가능 수량이 안 맞으면 한 단위 덜 사고 잔액은 현금으로 남긴다. 실제 펀드는 현금을 꽤 들고 운용한다. 첫 번째 절반(청구하는 채널에서 비용을 읽기)은 그대로 버그이고 더 무겁다 |
| `079` | **C — 데이터와 타입은 사용자 책임** | 스키마를 선언하게 하지 않고(A 기각), precision을 지목하지도 않는다(B 기각). 구분 못 하는 자리에서는 pyarrow 문장을 그대로 낸다. skill이 "연속량은 `float`"을 말한다 |
| `080` | **`081`의 선행 조건이 됐다** | 열거가 불완전한 채로 cascade를 얹으면 "보이는 것만 지우고 성공했다"가 된다 |
| `081` | **cascade를 만든다** | 삭제는 쉬워야 한다. `rm run --cascade`. 부분 실패는 되돌리지 않고 남은 것을 이름으로 보고한다. 작은 절반 둘(`list runs`의 고아 표시, `rm run-definition`의 payload)도 함께 |
| `085`·`086` | **후한 허용오차 + 기록을 셋으로** | 기본 `max(bound × 1%, 10bp)`, override 가능. 판정은 `constraints/evaluation.py` 한 자리 — 기존 constraint·scaffold·저자 코드 **변경 0**. 기록은 `held`/`within_tolerance`/`breached`와 각각의 worst excess. `085`도 같은 규칙(카운터를 나눈다, `never_filled`를 성공 payload에) |
