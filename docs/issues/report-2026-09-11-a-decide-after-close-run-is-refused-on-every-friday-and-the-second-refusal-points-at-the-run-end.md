...
**Status: CLOSED 2026-09-11 by record `259` — one classifier tells a `within` too short for a weekend (the fix names the longest wait and the smallest window that works, and says `within` is wall-clock time) from an end nothing is served before (the fix names the end between the last fill and that decision); both doors use it and `check` lists each occurrence once.** Received 2026-09-11 (접수) — confirmed against `develop` `ee2e2711`: two doors refuse the same occurrences: `flow/declaration/judgments.py::_judge_execution_ordering` (`execution.not_after_decision`) and `flow/declaration/preflight.py::_validate_execution_targets` (`execution.target_outside_horizon`) each list every occurrence `select_target` cannot bind, and the second's `fix` names the run end whether the missing instant lies inside the horizon or past it. Neither says what `within` measures or names the end record `237` describes.
