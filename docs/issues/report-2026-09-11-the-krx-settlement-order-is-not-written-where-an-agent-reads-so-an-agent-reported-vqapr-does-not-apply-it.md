...
**Status: HALF CLOSED 2026-09-11 — the skill half (skills-only commit, no record): make-exchange `references/execution-profiles.md` states the three steps (size toward zero, fund the largest money delta first, settle sells before buys) and what the record shows of each; analyze-result `references/panels-from-tables.md` says the same beside the fill columns. OPEN: a marker in the record when the planner cuts a buy for cash — it changes the record's shape and waits for the owner.** Received 2026-09-11 (접수) — confirmed against `develop` `ee2e2711`: sells settle before buys (`exchange/venues/krx.py::_settlement_order`), buys are funded largest money delta first (`exchange/planning.py::_buy_order`), and the fill batch is sorted back into instrument order before it is recorded (`krx.py`, "Back into identity order"); no shipped skill states either rule. The skill half is fixed directly; a trim marker in the record changes the record's shape and waits for the owner.
