...
## Step 2b — `061`: 창의 한 컬럼은 요청될 때 변환된다 (끼워 넣은 단계)

Step 2가 병합되는 사이 `0.3.0` 시나리오 testbed가 이슈 `055`–`066`을 냈다. 그중 `061`
(`PanelWindow.values[name]`이 창의 모든 컬럼을 변환한다, `counts()`가 읽기마다 같은 일을 한다)은
Step 2와 같은 읽기 경로이고 수정이 작아 여기 넣었다. record `143`. 나머지 열하나 중 `058`·`059`는
Step 5·7이 흡수하고, 그 외는 캠페인 뒤 "싸고 효과 좋은 순"으로 다룬다.
