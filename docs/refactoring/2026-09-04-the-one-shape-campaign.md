...
| **3** | `Rebalance.of`가 `rescale(grid=QUANTUM)`을 부른다. `Rebalance.signed(weights, *, gross=1)`. `Budget`·`Rebalance` docstring | `075` | `153` |
| **4** | `flow/materialize.py` 삭제, public에서 이름 넷, `stamping.py`→`datamodel.py`. showcase 005–008을 "member run → 표 등록 → ensemble run"으로. skill에 "run의 표를 등록한다" | — | `154` |
| **5** | **문서가 도메인이다** (D1·D2·D3, ExecPlan). `to_domain` 층·`flow/run.py` 앞 373줄·`Workspace.register_*`·`declarations.py`의 문장 생성기 삭제. `Frozen*`는 frozen pydantic | `027`(문서 메서드 한 줄) | `155` |
| **6** | **기록 family 7 → 3** (ExecPlan): `flow/frozen.py`·`flow/record.py`(모델 셋 + layout + writer/lock + reader)·`flow/run_state.py`(+`model_state`). `reporting`→`cli/run`. `valuation↔callback` 순환 절단 | — | `156` |
| **7** | 작은 패키지 접기: `domain/` 10→4, `constraints/` 7→3, `valuation/`→`flow/valuation` 옆, `runtime/`→`flow/`. 남은 순환 셋 | — | `157` |

순서의 제약: 4가 5 앞 — materialize가 `workspace.register_dataset` 직접 문의 마지막 외부 호출자다.
