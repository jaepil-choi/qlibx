...
the run's sessions, reporting the process's working set and its peak; `bounded` registers the
span as `[run start - lookback, run end]` to emulate a scan bounded by the run's horizon. To run
...
| develop, scan bounded to a 7-year run | | | 0.17 GB |
| develop, scan bounded to a 1-year run | | | 0.13 GB |
