...
class TestGenReturnsDfDedup:
    ...
    def test_nulls_return_above_10x(self, test_paths) -> None:
        """Post-correction returns > 1000% (ret > 10) are nulled; a return at
        the 10x bound is kept."""
        df = pl.DataFrame(
            {
                "gvkey": ["001"] * 4,
                "iid": ["01"] * 4,
                "datadate": [
                    date(2024, 1, 31),
                    date(2024, 2, 29),  # ret = 9.0  (<= 10, kept)
                    date(2024, 3, 29),  # ret = 11.0 (> 10, nulled)
                    date(2024, 4, 30),  # ret = 0.0  (kept)
                ],
                "eom": [
                    date(2024, 1, 31),
                    date(2024, 2, 29),
        ...
        assert feb["ret"][0] == pytest.approx(9.0, rel=1e-10)  # at bound, kept
        assert mar["ret"][0] is None  # > 10x, nulled
