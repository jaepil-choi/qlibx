    ...
    def run_targets(
        ...
        *,
        ...
        if matched is not None and target_policy is not None:
            raise ValueError(
                "matched_capitalization and target_policy cannot be used together"
            )
        if matched is not None and resume_state is not None:
            ...
            if any(value is None for value in matched_resume_values):
                raise ValueError(
                    "matched_capitalization resume_state is missing baseline state"
                )
        if (
            matched is None
            ...
            raise ValueError(
                "long-only resume_state must not contain matched capitalization state"
            )
        ...
        account, previous_nav, order_number, fill_number = _build_account(
            market, resume_state
        )
        ...
        if matched is not None and resume_state is not None:
            ...
            capitalization_event_sequence = int(
                resume_state.next_capitalization_event_sequence
            )
        else:
        ...
        for offset in range(start_position, stop_position):
            ...
            if matched is not None:
                intended_weight = matched.signed_weights.loc[date].where(
                ...
                if additions.gt(0).any():
                    ...
                    if baseline_cash < -ACCOUNT_RECONCILIATION_TOLERANCE:
                        raise RuntimeError(
                            "matched capitalization baseline cash became negative"
                        )
                        ...
                        capitalization_event_rows.append(
                            {
                                ...
                                "baseline_quantity_before": int(
                                    baseline_before[instrument]
                                ),
                                "baseline_quantity_after": int(
                                    baseline_quantity[instrument]
                                ),
                                "baseline_cash_before": float(
                                    baseline_cash_before
                                ),
                                "baseline_cash_after": float(
                    ...
                    current_adjusted = pd.Series(
                        {
                            instrument: account.current_position.get_stock_amount(
                                instrument
                            )
                            for instrument in weights.columns
                        },
                    ...
                    pretrade_cash = float(account.current_position.get_cash())
                    pretrade_market_value = current_physical_quantity.mul(price)
                    capitalized_nav = pretrade_cash + float(
                    ...
                    if not np.isclose(
                        ...
                        raise RuntimeError(
                            "matched capitalization changed composite NAV"
                        )
                raw_signed_target = intended_weight.mul(active_pretrade_nav).div(price)
                signed_target_quantity = _round_signed_target_quantity(
                    raw_signed_target, lot_size
                )
                raw_target_physical = baseline_quantity.astype("float64").add(
                    raw_signed_target
                )
                target_physical = baseline_quantity.add(
                ...
                if not isinstance(decided, pd.Series):
                    raise TypeError("target_policy must return a pandas Series")
                if decided.index.has_duplicates:
                    raise ValueError("target_policy returned duplicate physical instruments")
                unknown = decided.index.difference(weights.columns)
                missing = weights.columns.difference(decided.index)
                if len(unknown) or len(missing):
                    raise ValueError(
                        "target_policy physical instruments do not match execution axes: "
                        f"unknown={unknown.tolist()}, missing={missing.tolist()}"
                    )
                decided = decided.reindex(weights.columns)
                if decided.isna().any():
                    raise ValueError("target_policy must cover every physical instrument")
                if (decided < 0).any() or float(decided.sum()) > 1.0 + 1e-10:
                    raise ValueError(
                        "target_policy returned invalid physical weights: "
                        f"minimum={decided.min()}, sum={decided.sum()}"
                    )
                weights.loc[date] = decided.astype("float64")
            if matched is None:
                raw_target_physical = _raw_target_quantity(
                    weights.loc[date], pretrade_nav, price
                )
                target_physical = _round_target_quantity(
                    raw_target_physical, lot_size
                )
            ...
            order_specs = [
                (instrument, value)
                for instrument, value in delta.items()
                if value < -1e-12
            ] + [
                (instrument, value)
                for instrument, value in delta.items()
            ...
            for instrument, delta_adjusted in order_specs:
                ...
                requested_physical = int(
                    round(abs(float(delta_adjusted)) * float(factor[instrument]))
                )
                ...
                trade_val, trade_cost, qlib_trade_price = exchange.deal_order(
                    order,
                ...
                filled_physical = int(
                    round(float(order.deal_amount) * float(factor[instrument]))
                )
                reason = _fill_reason(
                    direction=direction,
                ...
                fill_rows.append(
                    {
                        ...
                        "quantity_after_tradability": (
                            diagnostic.amount_after_tradability
                            * float(factor[instrument])
                        ),
                        "quantity_after_volume": (
                            diagnostic.amount_after_volume
                            * float(factor[instrument])
                        ),
                        "quantity_after_position": (
                            diagnostic.amount_after_position
                            * float(factor[instrument])
                        ),
                        "quantity_after_cash": (
                            diagnostic.amount_after_cash
                            * float(factor[instrument])
                        ),
                        "quantity_after_lot": (
                            diagnostic.amount_after_lot
                            * float(factor[instrument])
                        ),
            ...
            if (
                matched is not None
                and matched.inventory_retention == "active_short_only"
            ):
                ...
                composite_after_fill = pd.Series(
                    {
                        instrument: int(
                            round(
                                float(
                                    account.current_position.get_stock_amount(
                                        instrument
                                    )
                                )
                                * float(factor[instrument])
                ...
                if releases.gt(0).any():
                    ...
                    baseline_quantity = baseline_quantity.sub(releases).astype(
                        "int64"
                    )
            ...
            for instrument in weights.columns:
                ...
                physical = int(round(float(adjusted) * float(factor[instrument])))
                physical_quantities[str(instrument)] = physical
                if physical == 0:
                    continue
                market_value = float(
                    physical * float(valuation_price[instrument])
                )
                    ...
                    signed_position_rows.append(
                        {
                            ...
                            "target_quantity": int(
                                signed_target_quantity[instrument]
                            ),
                ...
                baseline_account_rows.append(
                    {
                        ...
                        "return_denominator": float(
                            market["baseline_return_denominator"]
                        ),
                ...
                active_account_rows.append(
                    {
                        ...
                        "portfolio_return": active_money_pnl
                        / float(market["active_booksize"]),
                        "trade_cost": day_trade_cost,
        ...
        if not fills.empty:
            fills["filled_quantity"] = fills["filled_quantity"].astype("int64")
        else:
            fills = fills.astype({"filled_quantity": "int64"})
        ...
        result = BackendRunResult(
            ...
            research_rows=(
                pd.DataFrame() if research_rows is None else research_rows.copy()
            ),
            ...
            extra_tables=(
                ...
                if matched is None
                    ...
                    "baseline_account_daily": pd.DataFrame(
            ...
            evidence={
                ...
                "resume_state": _snapshot_state(
                    ...
                    previous_baseline_nav=(
                        None if matched is None else previous_baseline_nav
                    ),
                    previous_active_nav=(
                        None if matched is None else previous_active_nav
                    ),
...
def _normalize_scenario(scenario: Any) -> dict[str, Any]:
    ...
    if not np.isfinite(execution_price.to_numpy()).all() or (
        execution_price <= 0
    ).any().any():
    ...
    if (
        not np.isfinite(active_booksize)
        or active_booksize <= 0
        or active_booksize > booksize
    ):
        raise ValueError(
            "active_booksize must be finite, positive, and no greater than booksize"
        )
    ...
    if not valuation_price.index.equals(index) or not valuation_price.columns.equals(
        physical
    ):
        raise ValueError("valuation_price axes must match execution_price")
    if not np.isfinite(valuation_price.to_numpy()).all() or (
        valuation_price <= 0
    ).any().any():
    ...
    requested_sellable = optional_matrix(
        scenario.sellable, "sellable", True
    ).astype(bool)
    suspended = optional_matrix(
        getattr(scenario, "suspended", None), "suspended", False
    ).astype(bool)
    ...
    for kind in asset_class.unique():
        ...
        if not np.isfinite(list(cost_policy[str(kind)].values())).all() or any(
            ...
            raise ValueError(
                f"cost policy for {kind} must contain finite non-negative rates"
            )
    ...
    return {
        ...
        "baseline_return_denominator": (
            baseline_booksize if baseline_booksize > 0 else booksize
        ),
        "position_unit_factor": factor,
        "volume": volume,
        "buyable": buyable,
...
def _normalize_targets(
    targets: pd.DataFrame, execution_price: pd.DataFrame
) -> pd.DataFrame:
    ...
    normalized = matrix.reindex(
        columns=execution_price.columns, fill_value=0.0
    ).astype("float64")
...
def _required_baseline_additions(
    *,
    ...
    if not needs_short_capacity.any():
        return additions
    capacity_notional = (
        float(active_nav) * float(per_name_short_cap) * float(safety_multiplier)
    )
...
def _raw_target_quantity(
    ...
    return (
        ...
        * float(nav)
        / execution_price.astype("float64")
    )


def _round_target_quantity(
    raw_target_quantity: pd.Series, lot_size: pd.Series
) -> pd.Series:
    quantity = np.floor((raw_target_quantity + 1e-10) / lot_size) * lot_size
    return quantity.astype("int64")


def _round_signed_target_quantity(
    raw_target_quantity: pd.Series, lot_size: pd.Series
) -> pd.Series:
...
def _validate_resume_state(
    market: Mapping[str, Any], resume_state: ResumeState
) -> None:
    ...
    if unknown:
        raise ValueError(
            f"resume_state contains unknown physical instruments: {sorted(unknown)}"
        )
    ...
    if any(value is not None for value in matched_values):
        ...
        if not np.isfinite(matched_scalars).all():
            raise ValueError(
                "resume_state matched capitalization values must be finite"
            )
        ...
        for quantity in baseline_quantity.values():
            ...
            if (
                ...
                raise ValueError(
                    "resume_state baseline quantities must be non-negative integers"
                )
        if resume_state.next_position == 0:
            baseline_nav = float(market["baseline_booksize"])
        else:
            previous_mark = market["valuation_price"].iloc[
                resume_state.next_position - 1
            ]
        ...
        baseline_error = abs(
            baseline_nav - float(resume_state.previous_baseline_nav)
        )
        ...
        if baseline_error > baseline_tolerance:
            raise ValueError(
                "resume_state baseline cash, quantity and NAV do not reconcile"
            )
        projected_active_nav = float(resume_state.nav) - baseline_nav
        active_error = abs(
            projected_active_nav - float(resume_state.previous_active_nav)
        )
        active_tolerance = max(
            ACCOUNT_RECONCILIATION_TOLERANCE,
            abs(projected_active_nav)
            * ACCOUNT_RECONCILIATION_RELATIVE_TOLERANCE,
        )
        if active_error > active_tolerance:
            raise ValueError(
                "resume_state composite, baseline and active NAV do not reconcile"
            )
    if resume_state.next_capitalization_event_sequence < 0:
        raise ValueError(
            "resume_state capitalization event sequence must be non-negative"
        )
...
def _signal_rows(
    signals: Mapping[str, pd.DataFrame] | None, weights: pd.DataFrame
) -> pd.DataFrame:
    columns = [
        "trade_date",
        "signal_name",
    ...
    for name, matrix in signals.items():
        if (
            not matrix.index.equals(weights.index)
...
def _snapshot_state(
    ...
    *,
    ...
    quote_price = {
        instrument: float(position.get_stock_price(instrument))
    ...
    return ResumeState(
        ...
        baseline_cash=(
            None if baseline_cash is None else float(baseline_cash)
        ),
        baseline_quantity=(
            None
        ...
        previous_baseline_nav=(
            None
            if previous_baseline_nav is None
            else float(previous_baseline_nav)
        ),
        previous_active_nav=(
            None if previous_active_nav is None else float(previous_active_nav)
        ),
        next_capitalization_event_sequence=int(
            next_capitalization_event_sequence
        ),
    )
