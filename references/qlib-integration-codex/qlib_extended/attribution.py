...
def build_signed_attribution(
    ...
    *,
    ...
    if missing_signed:
        raise ValueError(
            f"signed_positions is missing columns: {sorted(missing_signed)}"
        )
    ...
    overnight_pnl = (
        prior_held * (execution_price - prior_valuation)
    ).fillna(0.0)
    ...
    if not fills.empty:
        ...
        grouped_cost = fills.groupby(
            ["trade_date", "instrument_id"], sort=False
        )["trade_cost"].sum()
        execution_cost = grouped_cost.unstack().reindex(
            index=execution_price.index,
            columns=execution_price.columns,
            fill_value=0.0,
        ).fillna(0.0)
    ...
    active_account = store.load_table(
        backtest_run_id, "active_account_daily"
    ).copy()
    required_account = {"trade_date", "money_pnl"}
    missing_account = required_account.difference(active_account.columns)
    if missing_account:
        raise ValueError(
            f"active_account_daily is missing columns: {sorted(missing_account)}"
        )
...
def _member_attribution(
    *,
    ...
    for member_run_id, weight in members.items():
        ...
        weighted[member_run_id] = (
            alpha.shift(signal_lag, fill_value=0.0).astype("float64") * weight
        )
    ...
    allocated = member_daily.groupby("trade_date")["net_pnl"].sum().reindex(
        execution_price.index
    )
