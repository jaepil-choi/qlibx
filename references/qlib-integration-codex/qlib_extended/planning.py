...
def build_plan(project: ProjectConfig, strategy_id: str) -> RunPlan:
    ...
    backtest_dataset_fingerprint = data.fingerprint(
        backtest_dataset_names(project.backtest)
    )
...
def execution_fingerprint_for_runtime() -> str:
    paths = []
    for module_name in (
        "qlib_extended.execution",
        "qlib_extended.portfolio",
        "kwam_qlib_backend.backend",
        "kwam_qlib_backend.exchange",
    ):
