...
def child_main(args: argparse.Namespace) -> int | None:
    if args.write_first:
        ...
        return emit(
            {
                "pid": os.getpid(),
                "changed": workspace.register_dataset(registration()),
    ...
    if args.write_second:
        ...
        return emit(
            {
                "pid": os.getpid(),
                "changed": workspace.register_dataset(registration("price_adjusted")),
    ...
    if args.conflict:
        ...
        try:
            workspace.register_dataset(registration(close="수정종가"))
        except VqaprError as error:
            return emit({"pid": os.getpid(), "error": error.as_dict()})
    ...
    if args.write_isolated:
        ...
        changed = workspace.register_dataset(registration("fundamentals"))
        return emit(
            {
                "pid": os.getpid(),
                "changed": changed,
...
def main() -> int:
    ...
    diagnosis, timing = validate(
        registration(), SourceSpec.of("fng_prices", DEV, hive_partitioned=True)
    )
