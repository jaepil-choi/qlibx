...
Every occurrence that computes a signal records the value *before* weighting (the ranked reversal)
...
- **Mark rehydration is scoped to what a later callback actually witnessed.** A committed
  ...
  such later occurrence, so its mark is legitimately absent from the published series and is
...
- **No warm-up trimming.** The signal needs six closes; earlier occurrences on the agenda decline
...
`instrument`, `signal_before_weighting`, `neutralized_signal`. It is appended to on every occurrence
with enough closes to compute a reversal view — including occurrences whose neutralised signal
turned out flat and returned `NoDecision` — because a run that recorded only its trading occurrences
...
| Metric | Value |
|---|---|
| signal occurrences | 16 of 21 callbacks (five decline for want of a full six-close window) |
