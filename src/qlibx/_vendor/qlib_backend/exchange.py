...
class ScenarioExchange(Exchange):
    ...
    def deal_order(
        ...
        try:
            ...
            self._last_diagnostic = _finalize_diagnostic(builder, float(order.deal_amount))
            return result
        finally:
    ...
    def _calc_trade_info_by_order(self, order, position, dealt_order_amount):
        ...
        try:
            result = super()._calc_trade_info_by_order(order, position, dealt_order_amount)
            ...
            if builder is not None:
                ...
                if order.direction == Order.BUY:
                    ...
                    if position is not None and position.check_stock(order.stock_id):
                        current_amount = float(position.get_stock_amount(order.stock_id))
                    if builder.amount_after_position is None:
                        builder.amount_after_position = min(float(after_volume), current_amount)
                    planned_after_lot = (
                    ...
                    if _less(float(order.deal_amount), float(planned_after_lot)):
                        ...
                        builder.amount_after_cash = float(builder.amount_after_position)
            return result
        finally:
    ...
    def _tradability_reason(self, order) -> str | None:
        ...
        if self._quote_bool(order, "$suspended"):
            return "suspended"
        if order.direction == Order.BUY and self._quote_bool(order, "$upper_price_limit"):
            return "upper_price_limit"
        if order.direction == Order.SELL and self._quote_bool(order, "$lower_price_limit"):
...
def build_quote_frame(
    ...
    *,
    ...
    qlib_deal_price = execution_price.astype("float64") * position_unit_factor.astype("float64")
    close_price = execution_price if valuation_price is None else valuation_price
    qlib_close_price = close_price.astype("float64") * position_unit_factor.astype("float64")
...
def _build_policies(
    ...
    for asset_class, values in cost_policy.items():
        rates = {name: float(values[name]) for name in ("buy_rate", "sell_rate", "sell_tax")}
        if not np.isfinite(np.asarray(list(rates.values()))).all() or any(
            value < 0 for value in rates.values()
        ):
            raise ValueError(f"execution cost rates must be finite and non-negative: {asset_class}")
...
def _finalize_diagnostic(builder: _DiagnosticBuilder, filled_amount: float) -> ExecutionDiagnostic:
    ...
    if builder.tradability_reason is not None:
        ...
        after_volume = _bounded_stage(builder.amount_after_volume, after_tradability, "volume")
        after_position = _bounded_stage(builder.amount_after_position, after_volume, "position")
        after_cash = _bounded_stage(builder.amount_after_cash, after_position, "cash")
        planned_after_lot = _bounded_stage(builder.amount_after_lot, after_cash, "lot")
        if filled_amount > planned_after_lot + 1e-10:
            raise RuntimeError("Qlib filled amount exceeds the instrumented execution stages")
