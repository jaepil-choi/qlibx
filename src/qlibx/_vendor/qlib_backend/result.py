...
class BackendRunResult:
    ...
    def position_quantity(self, date: pd.Timestamp, instrument: str) -> int:
        ...
        if rows.empty:
            return 0
        match = rows.loc[
            rows["trade_date"].eq(pd.Timestamp(date)) & rows["instrument_id"].eq(instrument),
            "held_quantity",
        ]
    ...
    def orders(self, date: pd.Timestamp | None = None) -> pd.DataFrame:
        ...
        return self.order_rows.loc[self.order_rows["trade_date"].eq(pd.Timestamp(date))].copy()

    def fills(self, date: pd.Timestamp | None = None) -> pd.DataFrame:
        if date is None or self.fill_rows.empty:
            return self.fill_rows.copy()
        return self.fill_rows.loc[self.fill_rows["trade_date"].eq(pd.Timestamp(date))].copy()
...
def _update_frame_hash(digest: Any, name: str, frame: pd.DataFrame) -> None:
    ...
    values = pd.util.hash_pandas_object(normalized.reset_index(drop=True), index=False)
