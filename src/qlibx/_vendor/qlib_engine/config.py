...
def load_project_config(path: str | Path) -> ProjectConfig:
    ...
    execution = ExecutionConfig(
        volume=(
            None if execution_raw.get("volume") is None else _required_str(execution_raw, "volume")
        ),
...
def _validate_backtest(config: BacktestConfig) -> None:
    ...
    if participation is not None and not 0 < participation <= 1:
        raise ConfigurationError("backtest.execution max_volume_participation must be in (0, 1]")
