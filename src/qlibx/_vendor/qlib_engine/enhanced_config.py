...
def parse_enhanced_index_config(
    ...
    *,
    ...
    return EnhancedIndexConfig(
        lookthrough=_nested_float_mapping(raw, "lookthrough", error_type=error_type),
        lower_bounds=_float_mapping(raw, "lower_bounds", error_type=error_type),
        upper_bounds=_float_mapping(raw, "upper_bounds", error_type=error_type),
        transaction_cost=_float_mapping(raw, "transaction_cost", error_type=error_type),
...
def validate_enhanced_index_config(
    ...
    *,
    ...
    if target_semantics == "enhanced_index" and config is None:
        raise error_type("enhanced_index target_semantics requires enhanced_index config")
    if target_semantics != "enhanced_index" and config is not None:
        raise error_type("enhanced_index config requires enhanced_index target_semantics")
    if config is None:
        return
    ...
    if not 0 <= config.cash_lower <= config.cash_upper <= 1:
        raise error_type("enhanced_index cash bounds must satisfy 0 <= lower <= upper <= 1")
...
def _required_mapping(
    ...
    *,
    ...
    if key not in value or not isinstance(value[key], Mapping):
        raise error_type(f"backtest.enhanced_index.{key} must be a mapping")
    result = value[key]
    if not result:
        raise error_type(f"backtest.enhanced_index.{key} must not be empty")
    return result
...
def _float_mapping(
    ...
    *,
    ...
    try:
        ...
        raise error_type(f"backtest.enhanced_index.{key} must contain numeric values") from exc
    ...
    *,
    ...
    for row_name, row in raw.items():
        if not isinstance(row, Mapping) or not row:
            raise error_type(
                f"backtest.enhanced_index.{key}.{row_name} must be a non-empty mapping"
            )
        try:
            result[str(row_name)] = {str(column): float(item) for column, item in row.items()}
        except (TypeError, ValueError) as exc:
            raise error_type(
                f"backtest.enhanced_index.{key}.{row_name} must contain numeric values"
            ) from exc
    return result
