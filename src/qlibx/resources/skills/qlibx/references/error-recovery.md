...
| NEXT_SESSION_OPEN_SCHEDULE_REQUIRED | A next-open profile has no explicit open-event calendar | Supply sorted unique `session_opens`; do not substitute close timestamps |
| NEXT_SESSION_NOT_AVAILABLE | No selected execution event occurs after the frozen parent decision | Extend the explicit schedule or select a different frozen parent; do not execute before the decision |
| EXECUTION_SESSION_PRICE_MISSING | A required instrument has no finite positive price available at the selected execution event | Correct the exact price binding/coverage or its honest availability; never fall back from open to a later close |
