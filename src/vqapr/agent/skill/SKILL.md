...
timezone-aware, or the dataset carries no dated row at all. These are all fixed while *preparing*
the source, not while registering it. vqapr deliberately does not convert a naive timestamp for
you: only you know which instant a value means, and a wrong localisation is a silent
point-in-time leak rather than an error. Localize at the instant the row became knowable — a
daily close is knowable at that session's close in the venue's timezone, not at midnight. For
