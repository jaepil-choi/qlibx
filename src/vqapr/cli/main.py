...
_DESCRIPTIONS: dict[str, str] = {
    ...
    "run": (
        ...
        "\n\n"
        "Several run ids run several runs, in a single process or in parallel under --jobs. A "
        "refusal inside one run is that run's outcome: the others still run, and the envelope "
        "reports every run with a status (ok:false, stage run.strategy_failed, the failed run's "
        "refusal in its own block). While a run is executing, "
        "`vqapr list strategies --run <run-id>` shows its progress."
    ),
    "list": (
