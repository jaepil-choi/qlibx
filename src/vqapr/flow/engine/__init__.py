...
and its subpackages have the phases: `declaration/` for what preflight makes of a run, `run/`
for the two loops and their handlers (record `214`). But the three modules here sat in the root
beside the assembly while the phases imported them fifteen times, so the root was simultaneously
above its children and below them.
