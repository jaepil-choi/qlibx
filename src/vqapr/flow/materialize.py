...
def _lineage_payload(
    *,
    ...
    return {
        ...
        "component": {"component_id": component_id, "fingerprint": fingerprint},
        "output": {
            "dataset_id": str(spec.dataset_id),
            "source_id": source_id,
            "value_fields": list(spec.value_fields),
        },
...
def materialize(
    ...
    *,
    ...
    stem = _safe_stem(str(spec.dataset_id))
    ...
    with tempfile.TemporaryDirectory(prefix=".materialize-", dir=workspace.path.parent) as raw_tmp:
        ...
        try:
            table = pa.Table.from_pylist([dict(row) for row in stamped_rows])
            pq.write_table(
                table,
                temporary_output,
                compression="zstd",
        ...
        candidate_registration = DatasetRegistration.of(
            ...
            fields={field: field for field in spec.value_fields},
        )
