...
def _agenda(from_dataset: str, *, role: str = "strategy_callback", at: str = "04:00") -> dict:
    return {"role": role, "from_dataset": from_dataset, "at": at, "timezone": "Asia/Seoul"}
...
def test_a_document_refused_at_its_kth_item_leaves_the_workspace_byte_identical(
    ...
    apply({"datasets": {"price_a": _dataset("src-a", model_price_parquet)}}, tmp_path, base=tmp_path)
    before = _fingerprint(tmp_path)
    assert before, "the first document must have created a workspace to protect"

    # Item 1 (a second dataset) is valid; item 2 (an agenda) names a dataset nobody declared.
    refused = {
        "datasets": {"price_b": _dataset("src-b", model_price_parquet)},
        "agendas": {"alpha": _agenda("nonexistent")},
    }
...
def test_a_valid_document_is_one_write(
    ...
    """Two datasets and two agendas, one of which follows a dataset declared in the same document."""
    writes: list[Path] = []
    ...
    document = {
        ...
        "agendas": {
            "alpha": _agenda("price_a"),
            "valuing": _agenda("price_b", role="valuation", at="16:00"),
        },
    }
    registered = apply(document, tmp_path, base=tmp_path)

    assert registered["datasets"] == ["price_a", "price_b"]
    assert registered["agendas"] == ["alpha", "valuing"]
    ...
    assert {agenda.agenda_id for agenda in reopened.agendas} == {"alpha", "valuing"}
...
def test_an_idempotent_document_writes_nothing(
    ...
    original = workspace_module.atomic.write_atomically
    monkeypatch.setattr(
        workspace_module.atomic,
        "write_atomically",
        lambda target, payload, **kw: (writes.append(Path(target)), original(target, payload, **kw)),
    )
