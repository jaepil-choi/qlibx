...
def test_availability_not_event_date_controls_no_look_ahead() -> None:
    ...
    assert leak.value.code == "QLIBX_BOUNDARY_LOOK_AHEAD"
    ...
    assert by_index.value.code == "QLIBX_BOUNDARY_LOOK_AHEAD"
...
def test_child_cannot_change_parent_observation_or_account() -> None:
    ...
    assert result.diagnostics["error_code"] == "QLIBX_BOUNDARY_CHILD_OBSERVATIONS"
    assert parent.account == {"cash": 100.0}
...
def test_feedback_order_and_resume_match_uninterrupted_results() -> None:
    ...
    assert unconfirmed.value.code == "QLIBX_BOUNDARY_FEEDBACK_UNCONFIRMED"
